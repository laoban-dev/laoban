...
const defaultLaobanConfig = (): LaobanConfig => ({
    packageManager: "yarn",
...
function normaliseLaobanConfig(
    ...
    mergeOptions?: Parameters<typeof mergeAll>[1]
): LaobanConfig {
    const merged = mergeAll(
        [defaultLaobanConfig(), config],
        mergeOptions
...
function addDiagnosticContextToErrors<T, E extends BaseIssue>(
    ...
    const first = addDiagnosticContextToIssue(result.errors[0] as LoaderIssue, diagnosticContext);
    const rest = result.errors.slice(1).map(e => addDiagnosticContextToIssue(e as LoaderIssue, diagnosticContext));
    const warnings = (result.warnings ?? []).map(w => addDiagnosticContextToIssue(w as LoaderIssue, diagnosticContext));
...
export async function loadConfigTreeFromFile(
    ...
    const oneFile = await loadAndValidateOneConfigFile(config, file, currentDiagnosticContext);
